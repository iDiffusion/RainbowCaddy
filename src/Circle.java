 ...
 * Author: Eli Rhyne & Liam Marshall
...
public class Circle {
	...
	public Circle(double radius, Point center, int numSpokes, ArrayList<Point> points) {
		...
		ring = pushCircle(ring, center, radius, points);
	}
    ...
    public void assignHeights(ArrayList<Point> ring, ArrayList<Point> points) {    /** Assign Heights - */
    	boolean found = false;
		    	...
		    	if(xCoords) {															/*If looking for x coords*/
		    		if(temp1.size() == 1) {
			    		...
			    		if(point.getX() > temp1.get(temp1.size()/2 + 1).getX()) {				/*if looking for point x is larger*/
			    			...
			    			Algorithms.quicksortX(points, 0, (temp1.size()-1));
			    			/*sort*/
			    		}
		    		...
		    		if(temp1.size() % 2 == 1) {											/*If Odd*/
			    		if(point.getY() > temp1.get(temp1.size()/2 + 1).getY()) {			/*if looking for point y is larger*/
			    			...
			    			Algorithms.quicksortY(points, 0, (temp1.size()-1));
			    			/*sort*/
			    		}
    ...
    public ArrayList<Point> pushCircle (ArrayList<Point> circle, Point cup, double radius, ArrayList<Point> points){
        int count;
        double deltaz;
