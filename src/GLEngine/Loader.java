...
public class Loader {
	...
	public RawModel loadToVao(float[] positions, float[] textureCoords, float[] normals, int[] indices, float[] colors, List<Vector3f> vertices) {
		int vaoID = createVao();
		...
		storeDataInAtrributeList(3, 3, colors);
		unbindVao();
	...
	public int loadTexture(String fileName) {
		...
		try {
			texture = TextureLoader.getTexture("JPG", new FileInputStream("res/model/" + fileName+".jpg"));
		} catch (IOException e) {
			e.printStackTrace();
		}
